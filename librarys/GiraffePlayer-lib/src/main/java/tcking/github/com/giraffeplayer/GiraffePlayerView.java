...
import com.giraffeplayer.lib.ui.R;
 ...
 */
public class GiraffePlayerView extends RelativeLayout {
    /**
    ...
    public ImageView iv_tmpimg_video;
    ...
    public GiraffePlayerView(Context context) {
        super(context);
        initGiraffePlayerView();
    }

    public GiraffePlayerView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        initGiraffePlayerView();
    }

    public GiraffePlayerView(Context context, AttributeSet attributeSet, int i) {
    ...
    private void initGiraffePlayerView() {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.giraffe_player, this);

        screenWidthPixels = getContext().getResources().getDisplayMetrics().widthPixels;
        $ = new Query();
        videoView = (IjkVideoView) findViewById(R.id.video_view);
        iv_tmpimg_video = (ImageView) findViewById(R.id.iv_tmpimg_video);
...
//                        Toaster.show("download rate:" + extra);
        ...
        initHeight = findViewById(R.id.app_video_box).getLayoutParams().height;
        hideAll();
        if (!playerSupport) {
            showStatus(getResources().getString(R.string.not_support));
        }
    }
    ...
    private void updatePausePlay() {
        if (videoView.isPlaying()) {
            $.id(R.id.app_video_play).image(R.mipmap.ic_stop_white_24dp);
        } else {
            $.id(R.id.app_video_play).image(R.mipmap.ic_play_arrow_white_24dp);
        }
    }
    ...
    private void statusChange(int newStatus) {
        ...
        if (!isLive && newStatus == STATUS_COMPLETED) {
            ...
            hideAll();
            if (isLive) {
                showStatus(getResources().getString(R.string.small_problem));
    ...
    public void setVideoPath(String url) {
        this.url = url;
        if (playerSupport) {
            iv_tmpimg_video.setVisibility(View.VISIBLE);
            $.id(R.id.app_video_replay).visible();
            videoView.setVideoPath(url);
        }
    }
...
//                            ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT;
    ...
    private void onVolumeSlide(float percent) {
        ...
        // 显示
        $.id(R.id.app_video_volume_icon).image(i == 0 ? R.mipmap.ic_volume_off_white_36dp : R.mipmap.ic_volume_up_white_36dp);
        $.id(R.id.app_video_brightness_box).gone();
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            $.id(R.id.app_video_fullscreen).image(R.mipmap.ic_fullscreen_exit_white_36dp);
            if(mListener != null){
                mListener.OnLandscape();
            }
        } else {
            $.id(R.id.app_video_fullscreen).image(R.mipmap.ic_fullscreen_white_24dp);
            if(mListener != null){
    ...
    OnOrienChangeListener mListener;
    public void setOnOrienChangeListener(OnOrienChangeListener mListener){
        this.mListener = mListener;
    }
    public interface OnOrienChangeListener{
        void OnPortrait();
        void OnLandscape();
        void OnPlay();
    }
    ...
    public void start() {
        iv_tmpimg_video.setVisibility(View.GONE);
        videoView.start();
        if(mListener != null){
            mListener.OnPlay();
        }
    ...
    class Query {
        ...
        public Query id(int id) {
            view = GiraffePlayerView.this.findViewById(id);
            return this;
        }
     ...
     */
    public GiraffePlayerView seekTo(int msec, boolean showControlPanle) {
    ...
    public GiraffePlayerView forward(float percent) {
    ...
    public GiraffePlayerView playInFullScreen(boolean fullScreen) {
        if (fullScreen) {
            ((Activity)getContext()).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
            updateFullScreenButton();
        }
    ...
    public GiraffePlayerView onError(OnErrorListener onErrorListener) {
        this.onErrorListener = onErrorListener;
        return this;
    }

    public GiraffePlayerView onComplete(Runnable complete) {
        this.oncomplete = complete;
        return this;
    }

    public GiraffePlayerView onInfo(OnInfoListener onInfoListener) {
        this.onInfoListener = onInfoListener;
        return this;
    }

    public GiraffePlayerView onControlPanelVisibilityChang(OnControlPanelVisibilityChangeListener listener) {
     ...
     * @return
     */
    public GiraffePlayerView live(boolean isLive) {
        this.isLive = isLive;
        return this;
    }

    public GiraffePlayerView toggleAspectRatio() {
        if (videoView != null) {
            videoView.toggleAspectRatio();
        }
        return this;
    ...
    public GiraffePlayerView onControlPanelVisibilityChange(OnControlPanelVisibilityChangeListener listener) {
        this.onControlPanelVisibilityChangeListener = listener;
        return this;
    }
