package com.asking.pad.app.widget.doodle;
...
import com.asking.pad.app.widget.doodle.action.Action;
...
//        }
    ...
    OnScrollToListener mListener;
    public interface OnScrollToListener{
        void OnScrollTo(Transaction t);
        void OnStartTime();
    }

    public void setOnScrollToListener(OnScrollToListener mListener){
    ...
    public synchronized void onTransaction(List<Transaction> transactions) {
        ...
        if (cache.size() > 0) {
            setPenColor(cache.get(0).getLineColor());
            setPenRech(cache.get(0).getLineRech());
            onMultiTransactionsDraw(cache);
            cache.clear();
        }
    }

    private void setPenColor(long lingColor){
        int r = ((int)lingColor >> 16)&0x0000ff;
        int g = ((int)lingColor >> 8)&0x0000ff;
        int b = ((int)lingColor >> 0)&0x0000ff;
        this.playbackChannel.setColor(Color.rgb(r,g,b));
    }

    private void setPenRech(long lingRech){
        int w = ((int)lingRech >> 8)&0x0000ff;
        setPaintSize(w);
    }
        ...
        for (Transaction t : transactions) {
            switch (t.getStep()) {
                case Transaction.ActionStep.START:
                    ...
                    playbackChannel.action = new MyPath(t.getX() * getWidth(), t.getY() * getHeight(), playbackChannel
                            .paintColor, playbackChannel.paintSize);
                    playbackChannel.action.onStart(canvas);
                    break;
                case Transaction.ActionStep.MOVE:
                    if (playbackChannel.action != null) {
                        playbackChannel.action.onMove(t.getX() * getWidth(), t.getY() * getHeight());
                ...
                case Transaction.ActionStep.END:
                    ...
                    break;
                case Transaction.ActionStep.SCROLL_TO:
                    if (mListener != null) {
                        mListener.OnScrollTo(t);
                    }
                    break;
                case Transaction.ActionStep.START_TIME:
                    if (mListener != null) {
